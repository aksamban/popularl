...
class CustomWrapper(Wrapper):
    def __init__(self, env, embedder, z):
        super().__init__(env)
        box = Box(shape=(env.observation_space.shape[0]+embedder.embed_dim()+z.shape[0],), low=-1000.0, high=1000.0)
    ...
    def step(self, action):
        ...
        observation = torch.cat((torch.tensor(step_obs), self.embedding, self.z))
    ...
    def reset(self, **kwargs):
        ...
        self.trajectory = []
        self.all_traject = []
        return torch.cat((torch.tensor(original_obs), self.embedding, self.z)), info